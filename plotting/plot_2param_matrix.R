...
hdiffmat = matrix(nrow=npar, ncol=npar)
...
#     cat("\n")
  ...
  diff_h = numeric(npar)
  for (i in 1:npar){
    pks = find_peaks(x=h_scan[i,], m=3)
    if (length(pks) < 2){
      diff_h[i] = 0 #pks
    } else if (length(pks) == 2){
      diff_h[i] = abs(pks[1]-pks[2])
    } else {
      pksid = order(h_scan[i,pks], decreasing = T)[1:2]
      diff_h[i] = abs(pks[pksid[1]]-pks[pksid[2]])
    }
  }
