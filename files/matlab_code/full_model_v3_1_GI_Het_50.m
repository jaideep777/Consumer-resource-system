...
% resource 
r = 0.2 + 0.15*(2*perlin2D(nx,50)-1);       % growth rate (must be scalar or NxN matrix)
...
%    l_disperse = sign(R(points_1d) - RTc) < 0;
    ...
    for i=1:n_imitation_events
        ...
        id_who = randi(Nc);
        % select individual to be copied from the remaining population
        id_whom = randi(Nc-1);
        if id_whom == id_who    % make sure to not copy self
            id_whom = Nc;
        end
